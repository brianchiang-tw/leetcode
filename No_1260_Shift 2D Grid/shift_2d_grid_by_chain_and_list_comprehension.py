...
class Solution:
    def shiftGrid(self, grid: List[List[int]], k: int) -> List[List[int]]:
        ...
        if k == 0:
            ...
            # flatten grid to 1D array by native chain functor in itertools
            flatten_arr = list( chain(*grid ) )
            
            # right shift k times by taking advantage of slicing syntax
            flatten_arr = flatten_arr[ -k : ] + flatten_arr[ : -k]
            
            # construct grid of right shift k by list comprehension
            grid_shift_k = [ flatten_arr[ y*cols : (y+1)*cols ] for y in range(rows) ]
...
#
# The overhead in time is the list( chain( *grid) ) and list comprehension, which are of O( m * n )
