...
class Solution:
    def summaryRanges(self, nums: List[int]) -> List[str]:
		...
		# add dummy infinity for ease of tail number handling
        ...
        for number in nums:
            ...
            if start is None:
                ...
                # update previous and continue iteration
                prev = number
                continue
            
            else:
                
                # add previous interval into result
                if start == prev:
					# interval with one number only
                    cur_interval = str(start)
                    
                else:
					# interval with multiple contiguous numbers
                    cur_interval = str(start) + '->' + str(prev)
