...
class Solution:
    def countAndSay(self, n: int) -> str:
        ...
        for _ in range (1, n):
            ...
            # scan each character on iteration of n
            for char in current_sequence:
                
                if char == prev_character:
                    # if catch a character repetition, count
                    character_count += 1
                
                else:
                    # if repetition ends with char count > 0, generate and append corresponding sequence
                    if character_count > 0:
                        next_sequence += ( str(character_count) + prev_character )
                    
                    # update previous character
                    prev_character = char

                    # reset character_count to 1 for next iternation of char
                    character_count = 1
        ...
        # the last current_sequence is the final count-and-say sequence of n
        count_and_say_sequence = current_sequence
