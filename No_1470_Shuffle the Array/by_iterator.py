...
class Solution(object):
    def shuffle(self, nums, n):
		...
		# make iterator for x series and y series
        x_iter, y_iter = iter(x_series), iter(y_series)
        
        output = []
        for _ in range(n):
            
			# generate output by definition
            output.append(next(x_iter))
            output.append(next(y_iter))
...
## Time Complexity: O( n )
#
# The overhead in time is the cost of array copy and for loop iteration, which are of O( n ).
