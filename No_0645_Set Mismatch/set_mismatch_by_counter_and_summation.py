...
class Solution:
    def findErrorNums(self, nums: List[int]) -> List[int]:
        ...
        # find the repeated element from occurrence
        repeated_element = Counter( nums ).most_common(1)[0][0]
        
        size = len(nums)
        
        # perfect_sum = current sum of unique element + missing element
        perfect_sum = size * (size+1) // 2
        
        cur_sum = sum( set(nums) )
        
        # compute the missing element from summation formula
        missing_element = perfect_sum - cur_sum
